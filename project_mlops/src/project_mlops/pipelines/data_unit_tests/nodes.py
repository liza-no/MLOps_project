import logging
from typing import Any, Dict, Tuple

import numpy as np
import pandas as pd

from great_expectations.core import ExpectationSuite, ExpectationConfiguration
import great_expectations as gx

from pathlib import Path

from kedro.config import OmegaConfigLoader
from kedro.framework.project import settings



logger = logging.getLogger(__name__)


def get_validation_results(checkpoint_result):
    # validation_result is a dictionary containing one key-value pair
    validation_result_key, validation_result_data = next(iter(checkpoint_result["run_results"].items()))

    # Accessing the 'actions_results' from the validation_result_data
    validation_result_ = validation_result_data.get('validation_result', {})

    # Accessing the 'results' from the validation_result_data
    results = validation_result_["results"]
    meta = validation_result_["meta"]
    use_case = meta.get('expectation_suite_name')
    
    
    df_validation = pd.DataFrame({},columns=["Success","Expectation Type","Column","Column Pair","Max Value",\
                                       "Min Value","Element Count","Unexpected Count","Unexpected Percent","Value Set","Unexpected Value","Observed Value"])
    
    
    for result in results:
        # Process each result dictionary as needed
        success = result.get('success', '')
        expectation_type = result.get('expectation_config', {}).get('expectation_type', '')
        column = result.get('expectation_config', {}).get('kwargs', {}).get('column', '')
        column_A = result.get('expectation_config', {}).get('kwargs', {}).get('column_A', '')
        column_B = result.get('expectation_config', {}).get('kwargs', {}).get('column_B', '')
        value_set = result.get('expectation_config', {}).get('kwargs', {}).get('value_set', '')
        max_value = result.get('expectation_config', {}).get('kwargs', {}).get('max_value', '')
        min_value = result.get('expectation_config', {}).get('kwargs', {}).get('min_value', '')

        element_count = result.get('result', {}).get('element_count', '')
        unexpected_count = result.get('result', {}).get('unexpected_count', '')
        unexpected_percent = result.get('result', {}).get('unexpected_percent', '')
        observed_value = result.get('result', {}).get('observed_value', '')
        if type(observed_value) is list:
            #sometimes observed_vaue is not iterable
            unexpected_value = [item for item in observed_value if item not in value_set]
        else:
            unexpected_value=[]
        
        df_validation = pd.concat([df_validation, pd.DataFrame.from_dict( [{"Success" :success,"Expectation Type" :expectation_type,"Column" : column,"Column Pair" : (column_A,column_B),"Max Value" :max_value,\
                                           "Min Value" :min_value,"Element Count" :element_count,"Unexpected Count" :unexpected_count,"Unexpected Percent":unexpected_percent,\
                                                  "Value Set" : value_set,"Unexpected Value" :unexpected_value ,"Observed Value" :observed_value}])], ignore_index=True)
        
    return df_validation


def test_data(df):
    context = gx.get_context(context_root_dir = "//..//..//gx")
    datasource_name = "hotel_datasource"
    try:
        datasource = context.sources.add_pandas(datasource_name)
        logger.info("Data Source created.")
    except:
        logger.info("Data Source already exists.")
        datasource = context.datasources[datasource_name]

    suite_hotel = context.add_or_update_expectation_suite(expectation_suite_name="Hotel")

    #avoiding duplicates at rerun
    suite_hotel.expectations = []
    
    #add expectations to the data
    expectation_booking_id = ExpectationConfiguration(
    expectation_type="expect_column_values_to_be_unique",
    kwargs={
        "column": "booking_id"
    },
        )
    suite_hotel.add_expectation(expectation_configuration=expectation_booking_id)

    expectation_booking_status = ExpectationConfiguration(
    expectation_type="expect_column_to_exist",
    kwargs={
        "column": "booking_status"
    },
        )
    suite_hotel.add_expectation(expectation_configuration=expectation_booking_status)

    expectation_meal = ExpectationConfiguration(
    expectation_type="expect_column_distinct_values_to_be_in_set",
    kwargs={
        "column": "type_of_meal",
        "value_set" : ['Meal Plan 1', 'Meal Plan 2', 'Meal Plan 3', 'Not Selected']
    },
        )
    suite_hotel.add_expectation(expectation_configuration=expectation_meal)

    expectation_room = ExpectationConfiguration(
    expectation_type="expect_column_distinct_values_to_be_in_set",
    kwargs={
        "column": "room_type",
        "value_set" : ['Room_Type 1', 'Room_Type 2', 'Room_Type 3', 'Room_Type 4', 'Room_Type 5', 'Room_Type 6', 'Room_Type 7']
    },
        )
    suite_hotel.add_expectation(expectation_configuration=expectation_room)

    expectation_market_segment = ExpectationConfiguration(
    expectation_type="expect_column_distinct_values_to_be_in_set",
    kwargs={
        "column": "market_segment_type",
        "value_set" : ['Online', 'Offline', 'Corporate', 'Complementary', 'Aviation']
    },
        )
    suite_hotel.add_expectation(expectation_configuration=expectation_market_segment)    


    expectation_adults = ExpectationConfiguration(
        expectation_type="expect_column_values_to_be_between",
        kwargs={
            "column": "number_of_adults",
            "max_value": 4,
            "min_value": 0
        },
    )
    suite_hotel.add_expectation(expectation_configuration=expectation_adults)

    expectation_children = ExpectationConfiguration(
        expectation_type="expect_column_values_to_be_between",
        kwargs={
            "column": "number_of_children",
            "max_value": 10,
            "min_value": 0
        },
    )
    suite_hotel.add_expectation(expectation_configuration=expectation_children)
    
    expectation_week_nights = ExpectationConfiguration(
        expectation_type="expect_column_values_to_be_between",
        kwargs={
            "column": "number_of_week_nights",
            "max_value": 17,
            "min_value": 0
        },
    )
    suite_hotel.add_expectation(expectation_configuration=expectation_week_nights)          

    expectation_weekend_nights = ExpectationConfiguration(
        expectation_type="expect_column_values_to_be_between",
        kwargs={
            "column": "number_of_weekend_nights",
            "max_value": 7,
            "min_value": 0
        },
    )
    suite_hotel.add_expectation(expectation_configuration=expectation_weekend_nights)  

    expectation_lead_time = ExpectationConfiguration(
        expectation_type="expect_column_values_to_be_between",
        kwargs={
            "column": "lead_time",
            "max_value": 365.0,
            "min_value": 0
        },
    )
    suite_hotel.add_expectation(expectation_configuration=expectation_lead_time) 

    expectation_average_price = ExpectationConfiguration(
        expectation_type="expect_column_values_to_be_between",
        kwargs={
            "column": "average_price",
            "max_value": 500,
            "min_value": 0
        },
    )
    suite_hotel.add_expectation(expectation_configuration=expectation_average_price) 



    context.add_or_update_expectation_suite(expectation_suite=suite_hotel)

    data_asset_name = "test"
    try:
        data_asset = datasource.add_dataframe_asset(name=data_asset_name, dataframe= df)
    except:
        logger.info("The data asset alread exists. The required one will be loaded.")
        data_asset = datasource.get_asset(data_asset_name)

    batch_request = data_asset.build_batch_request(dataframe= df)


    checkpoint = gx.checkpoint.SimpleCheckpoint(
        name="checkpoint_ingested",
        data_context=context,
        validations=[
            {
                "batch_request": batch_request,
                "expectation_suite_name": "Hotel",
            },
        ],
    )
    checkpoint_result = checkpoint.run()

    df_validation = get_validation_results(checkpoint_result)
    
    
    #assertions to stop the pipeline

    pd_df_ge = gx.from_pandas(df)

    #a booking must have an identifier
    assert pd_df_ge.expect_column_values_to_not_be_null("booking_id").success == True
    #target column must exist
    assert pd_df_ge.expect_column_to_exist("booking_status").success == True
    #booking must be made for a future date (lead_time not negative)
    assert pd_df_ge.expect_column_values_to_be_between("lead_time", min_value=0).success

    log = logging.getLogger(__name__)
    log.info("Data passed on the unit data tests")
  
    
    return df_validation